...
Unit tests for the structured 5-component market sub-index aggregation.
...
components (returns, momentum, order_flow, liquidity) — short_volume is
absent. The missing short_volume weight (0.15) is redistributed across
the other 4 present components (sum = 0.85), so effective weights differ
...
def _full_signals(
    ...
    """Build signals for the 4 original components (short_volume absent)."""
    return [
...
#     (short_volume missing → weight redistributed across the 4 present)
# ---------------------------------------------------------------------------
...
class TestDisagreement:
    ...
    def test_returns_bullish_order_flow_bearish(self):
        ...
        4 components present (short_volume missing), sum=0.85:
        Effective: returns=0.35/0.85, momentum=0.15/0.85, order_flow=0.25/0.85, liquidity=0.10/0.85
        Combined = (0.35/0.85)*(+1) + (0.15/0.85)*(0) + (0.25/0.85)*(-1) + (0.10/0.85)*(0)
                 = (0.35 - 0.25) / 0.85 = 0.10/0.85 ≈ +0.1176
        Value = 50 + 50*0.1176 ≈ 55.88
        """
        sigs = _full_signals(
            return_score=100.0,     # → +1.0
        ...
        si = compute_market_sub_index(sigs)
        assert si is not None
        assert abs(si.value - 55.88) < 0.1

    def test_returns_bearish_order_flow_bullish(self):
        """Opposite of above: returns=-1, order_flow=+1.

        Combined = (0.25 - 0.35) / 0.85 = -0.10/0.85 ≈ -0.1176
        Value = 50 + 50*(-0.1176) ≈ 44.12
        """
        sigs = _full_signals(
            return_score=0.0,       # → -1.0
            rsi_raw=50.0,           # → 0.0
...
class TestMissingOneComponent:
    ...
    def test_liquidity_missing_weight_redistributed(self):
        ...
        Present weights: returns=0.35, momentum=0.15, order_flow=0.25 → sum=0.75
        Effective: returns=0.35/0.75, momentum=0.15/0.75, order_flow=0.25/0.75

        All present at +1:
        combined = (0.35+0.15+0.25)/0.75 = 0.75/0.75 = 1.0
        value = 50 + 50*1.0 = 100
        """
        sigs = [
    ...
    def test_momentum_missing_weight_redistributed(self):
        ...
        Present: returns=0.35, order_flow=0.25, liquidity=0.10 → sum=0.70
        Effective: returns=0.50, order_flow=0.357, liquidity=0.143

        returns=+1, order_flow=+1, liquidity=0 (neutral):
        combined = 0.50*1 + 0.357*1 + 0.143*0 = 0.857
        value = 50 + 50*0.857 ≈ 92.86
        """
        sigs = [
            _sig("return_1d",  100.0),
            _sig("return_5d",  100.0),
        ...
        si = compute_market_sub_index(sigs)
        assert si is not None
        assert abs(si.value - 92.86) < 0.01

    def test_returns_missing_weight_redistributed(self):
        """Returns missing. Only momentum, order_flow, liquidity present.

        Present: momentum=0.15, order_flow=0.25, liquidity=0.10 → sum=0.50
        Effective: momentum=0.30, order_flow=0.50, liquidity=0.20

        All at +0.5:
        combined = 0.30*0.5 + 0.50*0.5 + 0.20*0.5 = 0.5*(0.30+0.50+0.20) = 0.5*1.0 = 0.5
...
# ---------------------------------------------------------------------------
# (e) 4 of 5 components missing → sub-index returns None
# ---------------------------------------------------------------------------

class TestFourMissing:
...
class TestRSIMomentumConvention:
    ...
    def test_rsi_70_maps_to_plus_one(self):
        ...
        4 components present (short_volume missing), sum=0.85:
        Effective momentum weight = 0.15/0.85 ≈ 0.1765
        combined = 0.1765 * 1.0 = 0.1765
        value = 50 + 50*0.1765 ≈ 58.82
        """
        sigs = [
...
class TestMetadata:
    ...
    def test_n_signals_counts_only_component_signals(self):
        sigs = _full_signals() + [_sig("volume_ratio", 60.0)]
        si = compute_market_sub_index(sigs)
        assert si is not None
...
# ---------------------------------------------------------------------------
# 5-component aggregation (all 5 including short_volume)
# ---------------------------------------------------------------------------
...
def _full_5_signals(
    ...
    """Build signals for all 5 components (short_volume included)."""
    return [
        _sig("return_1d",  return_score),
        _sig("return_5d",  return_score),
        _sig("return_20d", return_score),
...
class TestFiveComponentAggregation:
    """Tests with all 5 components present (no weight redistribution)."""

    def test_all_five_strongly_bullish(self):
        """All 5 components at +1.0 → value = 100."""
        sigs = _full_5_signals(
            return_score=100.0,    # → +1.0
            rsi_raw=70.0,          # → +1.0
            ofi_score=100.0,       # → +1.0
    ...
    def test_all_five_strongly_bearish(self):
        """All 5 components at -1.0 → value = 0."""
        sigs = _full_5_signals(
            return_score=0.0,      # → -1.0
            rsi_raw=30.0,          # → -1.0
            ofi_score=0.0,         # → -1.0
    ...
    def test_all_five_neutral(self):
        """All 5 at neutral → value = 50."""
        sigs = _full_5_signals()
        si = compute_market_sub_index(sigs)
        assert si is not None
        assert si.value == 50.0
    ...
    def test_short_volume_spike_others_flat(self):
        ...
        All 5 present, sum=1.0:
        combined = 0.35*0 + 0.15*0 + 0.25*0 + 0.10*0 + 0.15*(-1) = -0.15
    ...
    def test_sv_plus_returns_others_missing(self):
        """Short volume + returns present, 3 others missing.

        Present: returns=0.35, short_volume=0.15 → sum=0.50
        Effective: returns=0.70, short_volume=0.30

        Both at +0.5:
        combined = 0.70*0.5 + 0.30*0.5 = 0.50
        value = 50 + 50*0.5 = 75.0
        """
        sigs = [
    ...
    def test_n_signals_with_short_volume(self):
        """n_signals should count short_volume_ratio_otc as a component signal."""
        sigs = _full_5_signals()
        si = compute_market_sub_index(sigs)
        assert si is not None
        # 3 return_* + rsi_14 + order_flow + spread + short_volume = 7
        assert si.n_signals == 7

    def test_weights_sum_to_one(self):
        """Verify the 5 component weights sum to 1.0."""
